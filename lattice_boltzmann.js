...
function simulate() {
    ...
    if (mouseIsDown && mouseSelect.selectedIndex==2) {
        if (oldMouseX >= 0) {
            ...
            pushX = gridLoc.x;
            pushY = gridLoc.y;
            pushUX = (mouseX - oldMouseX) / config.pxPerSquare / stepsPerFrame;
            pushUY = -(mouseY - oldMouseY) / config.pxPerSquare / stepsPerFrame;	// y axis is flipped
...
//function colorSquare(x, y, cIndex) {		// for some strange reason, this version is quite a bit slower on Chrome
    ...
    for (var py=flippedy*config.pxPerSquare; py<(flippedy+1)*config.pxPerSquare; py++) {
        for (var px=x*config.pxPerSquare; px<(x+1)*config.pxPerSquare; px++) {
